...
namespace KeyPayV2.Nz.Functions
{
    public class OtherFunction : BaseFunction
    {
        ...
        {
            return ApiRequest<NzPayRunDetailsModel>($"/business/{businessId}/payrun/{payRunId}/details");
        }
        ...
        {
            return ApiRequest<List<TasksReportExportModel>>($"/business/{businessId}/report/tasks?employeeId={request.EmployeeId}&payRunId={request.PayRunId}&fromDate={request.FromDate}&toDate={request.ToDate}&payScheduleId={request.PayScheduleId}&status={request.Status}");
        }
    }
}
